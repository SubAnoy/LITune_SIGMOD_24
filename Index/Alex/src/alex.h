...
class Alex {
 ...
 public:
  ...
  struct Params {
    ...
    double expected_insert_frac = external_expectedInsertFrac; //1;
    // Maximum node size, in bytes. By default, 16MB.
    // Higher values result in better average throughput, but worse tail/max
    // insert latency
    int max_node_size = external_maxNodeSize; //1 << 24;
    // Approximate model computation: bulk load faster by using sampling to
    // train models
    bool approximate_model_computation = external_approximateModelComputation; //true;
    // Approximate cost computation: bulk load faster by using sampling to
    // compute cost
    bool approximate_cost_computation = external_approximateCostComputation; //false;
  };
  Params params_;
  ...
  struct DerivedParams {
    ...
    int max_fanout = external_maxNodeSize / sizeof(void*); //1 << 21;  // assumes 8-byte pointers
    int max_data_node_slots = (external_maxNodeSize) / sizeof(V); //(1 << 24) / sizeof(V);
  };
  ...
  struct ExperimentalParams {
    ...
    int fanout_selection_method = external_fanoutSelectionMethod; //0;
    ...
    int splitting_policy_method = external_splittingPolicyMethod; //1;
    ...
    bool allow_splitting_upwards = external_allowSplittingUpwards; //false;
  };
 ...
 private:
  ...
  static const int kMinOutOfDomainKeys = external_kMinOutOfDomainKeys; //5;
  // After this many keys are outside the domain, a domain expansion must be
  // triggered.
  static const int kMaxOutOfDomainKeys = external_kMaxOutOfDomainKeys; //1000;
  ...
  // factor.
  static const int kOutOfDomainToleranceFactor = external_kOutOfDomainToleranceFactor; //2;
